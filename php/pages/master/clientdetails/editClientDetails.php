							   ...
							   <div class="span3"><label>Category</label><input type="text" value="<?php echo $rows['CLIENT_CATEGORY'];?>" disabled="disabled" class="span3"></div>
							    ...
							    <div class="span6"><label>SE Code<font color="#FF0000"> *</font></label>
								 ...
								 <?php 
											...
											echo '<select class="span6" name="SE_CODE" onChange="showStates(this);">';
																						
											for($i=0; $rowjob = $result->fetch(); $i++){																	
												Database::disconnect();	
								?>												
											
											<option value="<?php  echo $rowjob['SE_CODE']; echo '" '; if($rowjob['SE_CODE'] == $row['SE_CODE']) { echo 'selected'; } ?> "><?php  echo $rowjob['SE_CODE'];?> ( <?php  echo $rowjob['SE_DESCRIPTION'];?> )  </option>  
											<?php } echo '</select>';  ?>
							    ...
							    <div id="RIcode">
								 <div class="span5">
								 <?php
								           $ID = $row['SE_CODE']; /** get the EMP_DETAILS ID **/
											$pdo = Database::connect();
											$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);			
											$result = $pdo->prepare("SELECT RI_CODE, RI_DESCRIPTION FROM JOB_HIERARCHY WHERE SE_CODE = '$ID'");
											$result->bindParam(':userid', $ID);
											$result->execute();
											$rowsss = $result->fetch();	
								 ?>
								<div class=""><label>RI Code</label><input type="text" value="<?php echo $rowsss['RI_CODE'];?> (<?php echo $rowsss['RI_DESCRIPTION'];?>)" name="RI_CODE" id="RI_CODE" disabled="disabled"  class="span5"></div>
								</div>	</div>
								<div class="span6"><label>SE Visit Schedule<font color="#FF0000"> *</font></label>
								 <?php 
											...
											echo '<select class="span6" name="SE_VISIT_SCH_PERIODID">';
																						
											for($i=0; $rowse = $result->fetch(); $i++){																	
												Database::disconnect();	
								?>	
								      <option value="<?php  echo $rowse['ID']; echo '" '; if($rowse['ID'] == $row['SE_VISIT_SCH_PERIOD_ID']) { echo 'selected'; } ?> "><?php  echo $rowse['SCH_PERIOD'];?></option>  
											<?php } echo '</select>';  ?>
								...
								<div class="span5"><label>RI Visit Schedule<font color="#FF0000"> *</font></label>
									...
									<?php 
											...
											echo '<select class="span5" name="RI_VISIT_SCH_PERIODID">';
																						
											for($i=0; $rowri = $result->fetch(); $i++){																	
												Database::disconnect();	
								?>												
										
											<option value="<?php  echo $rowri['ID']; echo '" '; if($rowri['ID'] == $row['RI_VISIT_SCH_PERIOD_ID']) { echo 'selected'; } ?> ><?php  echo $rowri['SCH_PERIOD'];?> ( <?php  echo $rowri['DESCRIPTION'];?> )  </option>  
											<?php } echo '</select>';  ?>
                                </div>								
								
								<div class="span3"><label>SE Visit Count <font color="#FF0000"> *</font></label><input type="text" maxlength="4" value="<?php echo $row['SE_VISIT_COUNT'];?>" name="SE_VISIT_COUNT"  id="SE_VISIT_COUNT" onkeypress="return onlyNos(event,this);" class="span3"></div>
								<div class="span3"><label>RI Visit Count<font color="#FF0000"> *</font></label><input type="text" maxlength="4" value="<?php echo $row['RI_VISIT_COUNT'];?>" name="RI_VISIT_COUNT" id="RI_VISIT_COUNT" onkeypress="return onlyNoss(event,this);"  class="span3"></div>
